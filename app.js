...
function checkPickups(){ if (!carrying){ for(const p of pickupTargets){ if(!p.taken && dist(pose.x, p.y, p.x, p.y) < 34){ p.taken=true; carrying=true; break; } } } else { if (dist(pose.x, pose.y, scoreZone.x, scoreZone.y) < scoreZone.r){ carrying=false; pickupsDone += 1; updateHUD(); } } }
